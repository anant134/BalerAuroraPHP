<?php
    ...
    switch ($requestfor) {
         ...
         case 'getport':
            ...
            $qr = excecutequery("call sp_getport('".$query."')");
        ...
        case 'port':
            ...
            $qr = excecutequery("call sp_port('".$query."')");
            while ($row = $qr->fetch_assoc()) {
                $id =$row['rowid'] ;
                $errflag =$row['errflag'] ;
                // $id = $row['rowid'];
        ...
        case 'getdeviceinfo':
            $query=json_encode($params["data"]);
            $qr = excecutequery("call sp_getdeviceinfo('".$query."')");
         //   print("call sp_getdeviceinfo('".$query."')");
            while ($row = $qr->fetch_assoc()) {
                $id = $row['id'];
                $isactive = $row['isactive'];
            ...
            echo json_encode($result[0]);
        break;
        case 'portdevicemapping':
            $query=json_encode($params["data"]);
            $qr = excecutequery("call sp_portdevicemapping('".$query."')");
           // print("call sp_portdevicemapping('".$query."')");
            while ($row = $qr->fetch_assoc()) {
                $id =$row['rowid'] ;
                $errflag =$row['errflag'] ;
                // $id = $row['rowid'];
