...
switch ($requestfor) {
    ...
    case 'registration':
        ...
        if (!(empty($return_arr))) {
            ...
            $qrfilname= genqr('qrr_','r_'.$return_arr[0]["rowid"]);
           // print_r($qrfilnamev);
            for ($i=0; $i <count($dasa["touristvehicles"]) ; $i++) { 
                
                
               
                // genqr('qrr_','r_'.$return_arr[0]["rowid"]);
            }
            if($json['results']['message']=='Success.'){
                // $conn->next_result();
                $qr = excecutequery("call sp_updatepaymentreferencenumber('" . $json['results']['data']['reference_number']. "','".$return_arr[0]["rowid"]."','".$qrfilname."')");
